...
if(in_array('woocommerce/woocommerce.php', apply_filters('active_plugins', get_option('active_plugins'))))
{
	...
	class WC_Favorites_Grampus
	{
		...
		public function favorites_items_add()
		{
			...
			$message = sprintf( '%s добавлен в избранные', $product->get_title() );
		...
		public function favorites_items_remove()
		{
			...
			$message = sprintf( '%s удален из избранных', $product->get_title() );
			wc_add_notice( $message, 'success' );
		...
		public function check_on_isset_products() 
		{
			if(!empty(self::$FAVORITES)) {				
				foreach(self::$FAVORITES as $key => $item) {
					$product = wc_get_product($item);

					if($product == false || get_post_status($item) == 'trash') {
						unset(self::$FAVORITES[$key]);
					}
				}
				self::saveSession();
