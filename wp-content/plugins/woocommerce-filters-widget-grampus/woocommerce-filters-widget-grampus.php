	...
	class WC_Filters_Widget_Grampus
	{
		...
		public function product_query($query)
		{
			...
			if(is_product_taxonomy() or is_shop())
			{
				...
				if (isset($http_request['width_before']) && isset($http_request['width_after'])) {

					$queryWidth[] = [
							'key'     => 'width',
							'value'   => array( $http_request['width_before'], $http_request['width_after'] ),
							'type'    => 'numeric',
							'compare' => 'BETWEEN'
						];

					$meta_query = array_merge($queryWidth, $meta_query);
