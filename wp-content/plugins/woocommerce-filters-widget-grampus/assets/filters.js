jQuery(function($) {
	...
	// ItemFilter.prototype.initSliderWidth = function() {
	// 	const maxWidthNub = parseFloat(document.querySelector('.width_after').getAttribute('data-help-max'));
	// 	const range = document.querySelector('[data-filter-node="width"]');
	// 	const inputMin = document.querySelector('.width_before'); 
	// 	const inputMax = document.querySelector('.width_after'); 
	// 	const min = inputMin.dataset.min;
	// 	const max = inputMax.dataset.max;
	...
	ItemFilter.prototype.initSliderWidth = function() {
		let attributes = document.querySelectorAll('.range-slider[data-filter-node]:not([data-filter-node="slider"])');
		if(attributes.length > 1) {
			attributes.forEach(element => {
				let name = element.dataset.filterNode;
				let maxWidthNub = parseFloat(element.parentElement.querySelector(`.${name}_after`).getAttribute('data-help-max'));
				let range = element;
				let inputMin = element.parentElement.querySelector(`.${name}_before`); 
				let inputMax = element.parentElement.querySelector(`.${name}_after`); 
				let min = inputMin.dataset.min;
				let max = inputMax.dataset.max;
